Cypress.Commands.add('testSetup', (index = 0) => {
  ...
  cy.exec('wp-env run tests-cli wp db import "test_run_dump.sql"', options).then((result) => {
    ...
    cy.exec('wp-env run tests-cli wp option delete list_values');
    ...
    cy.exec('wp-env run tests-cli wp plugin activate s3-uploads disable-user-login;')
    ...
    cy.exec('wp-env run tests-cli wp db export "test_run_dump.sql"')
  });
});