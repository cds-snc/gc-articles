...
class NotifySettings
{
    ...
    public function notifyGenericTemplateIdCallback()
    {
        ...
        </details>', __('Enter your generic Email Template ID', 'cds-snc'), __('See example template ID format.', 'cds-snc'));

        $templateTitle = __("Example Template", "cds-snc");
        $copyBtnText =  __('Copy template content to clipboard', 'cds-snc');
        printf(
            '<div><h3>%s</h3><img alt="" class="gc-list-template-img" src="%s"><div><button type="button" class="button gc-list-template-btn" id="copy-template">%s</button><div></div>',
            $templateTitle,
            $this->templateImage(),
            $copyBtnText
        );
    ...
    public function templateImage(): string
    {
        return cds_plugin_images_url("gc-list-template.png");
    }
}